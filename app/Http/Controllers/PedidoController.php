...
class PedidoController extends Controller
{
    ...
    public function webhook(Request $request)
    {
        ...
        $endpointSecret = "whsec_d060f20eb52a7b859447795fb7f141dbb310e7276fc5c697c167e86c57f0427f";
        $event = Event::constructFrom($request->all());
        switch ($event->type) {
            case "payment_intent.succeeded":
                $pedido = Pedido::where("payment_intent_id", $event->data->object->id)->first();
                if (!$pedido){
                    return response()->json(["msg" => "Pedido nao encontrado"]);
                }
                ...
                break;
            default:
            return response()->json(["msg" => "Evento {$event->type} nao esta classificado"]);
    }
}
}
